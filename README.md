...
## Dane przykładowe i publikacja w Git

`test_data/` zawiera małe, opisane zestawy referencyjne użyte do analizy
formatu danych i testu ściany. Surowe pakiety, macierze NumPy i bufory YUV są
domyślnie ignorowane przez Git, aby nowe przechwycenia nie powiększały
repozytorium bez świadomej decyzji. Jeżeli zestaw danych ma być opublikowany,
warto pozostawić jego `README.md`, metadane i podglądy PNG, a duże dane
udostępnić jako osobny załącznik lub przez Git LFS.

Ignorowane są też lokalne katalogi `logs/`, `captures/`, `exports/`,
`pointclouds/` i `recordings/`. Mogą one zawierać duże pliki i obraz
rzeczywistej sceny. Archiwa oraz `camera_original/` są lokalnymi kopiami
oprogramowania/zasobów urządzenia; przed ich publicznym udostępnieniem należy
sprawdzić warunki licencyjne i redystrybucji Sipeed.

Przed pierwszym publicznym commitem sprawdź planowany zakres:

```bash
git init
git status --ignored --short
git add README.md requirements.txt .gitignore maixsense_probe.py a075v/ docs/ vendor/sipeed_a075v/
git diff --cached --check
```

Repozytorium nie zawiera jeszcze licencji dla autorskiego kodu. Przed
publikacją dodaj `LICENSE` zgodną z wybranym sposobem udostępniania (np. MIT
dla otwartego kodu) oraz upewnij się, że nie obejmuje ona plików producenta.
